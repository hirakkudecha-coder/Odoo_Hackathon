...
export const getAnalytics = async (req, res, next) => {
  try {
    ...
    // Aggregate trips (distance & revenue)
    const trips = await Trip.aggregate([
      { $match: { isDeleted: { $ne: true }, status: 'completed' } },
      { $group: { _id: '$vehicleId', totalDistance: { $sum: '$distanceKm' }, totalRevenue: { $sum: '$revenue' } } }
    ]);

    // Aggregate fuel logs (liters & cost)
    const fuelLogs = await FuelLog.aggregate([
      { $match: { isDeleted: { $ne: true } } },
      { $group: { _id: '$vehicleId', totalLiters: { $sum: '$liters' }, totalFuelCost: { $sum: '$cost' } } }
    ]);

    // Aggregate maintenance expenses
    const maintenanceExpenses = await Expense.aggregate([
      { $match: { isDeleted: { $ne: true }, category: 'Maintenance' } },
      { $group: { _id: '$vehicleId', totalMaintenanceCost: { $sum: '$amount' } } }
    ]);

    // Combine data
    const analyticsData = vehicles.map(vehicle => {
      const vId = vehicle._id.toString();
      
      const tripData = trips.find(t => t._id && t._id.toString() === vId) || { totalDistance: 0, totalRevenue: 0 };
      const fuelData = fuelLogs.find(f => f._id && f._id.toString() === vId) || { totalLiters: 0, totalFuelCost: 0 };
      const maintenanceData = maintenanceExpenses.find(m => m._id && m._id.toString() === vId) || { totalMaintenanceCost: 0 };

      const totalDistance = tripData.totalDistance;
      const fuelConsumed = fuelData.totalLiters;
      const fuelEfficiency = fuelConsumed > 0 ? (totalDistance / fuelConsumed) : 0;
      
      const revenue = tripData.totalRevenue;
      const fuelCost = fuelData.totalFuelCost;
      const maintenanceCost = maintenanceData.totalMaintenanceCost;
      const operationalCost = fuelCost + maintenanceCost;
      
      const acquisitionCost = vehicle.acquisitionCost || 1; // avoid div by 0
