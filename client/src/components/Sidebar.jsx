...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`glass-panel border-r shrink-0 min-h-screen transition-all duration-300 flex flex-col z-30 ${sidebarOpen ? 'w-64' : 'w-20'
        }`}
    >
      {/* Brand logo */}
      <div className={`h-16 flex items-center border-b border-slate-200/50 dark:border-slate-800/50 relative ${sidebarOpen ? 'justify-between px-5' : 'justify-center'
        }`}>
        ...
        <button
          ...
          className={`p-1 rounded-lg transition-colors z-50 ${sidebarOpen
            ? 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
            : 'absolute -right-3 top-1/2 -translate-y-1/2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-slate-700 hover:bg-slate-50 shadow-sm'
            }`}
        >
        ...
        {allowedNav.map((item) => {
          ...
          return (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium ${isActive
